    ...
    <div className="min-h-screen bg-brand-bg text-brand-black">
      <section className="relative h-[70vh] min-h-[600px] flex items-center justify-center overflow-hidden bg-brand-black">
        <div className="absolute inset-0 opacity-30">
          <img src={content.hero.imageUrl} alt="LUXARDO FASHION Global Partnerships" className="w-full h-full object-cover " referrerPolicy="no-referrer" />
        </div>
        <div className="absolute inset-0 bg-gradient-to-b from-brand-black/40 via-brand-black/60 to-brand-black" />
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center space-y-8">
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1] }} className="text-[11px] uppercase tracking-[0.4em] font-bold text-brand-white/70">{content.hero.subtitle}</motion.p>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} className="text-5xl md:text-7xl font-display tracking-wide text-brand-white">{content.hero.title}</motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, delay: 0.4, ease: [0.22, 1, 0.36, 1] }} className="text-xl md:text-2xl font-sans text-brand-white/80 max-w-2xl mx-auto leading-relaxed font-light">{content.hero.description}</motion.p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="space-y-12">
              <div className="space-y-6">
                ...
                <h2 className="text-4xl md:text-5xl font-display tracking-tight leading-tight">{content.trust.heading}</h2>
              </div>
              <p className="text-xl font-sans text-brand-secondary leading-relaxed font-light">{content.trust.text}</p>
              <div className="grid grid-cols-2 gap-12 pt-8 border-t border-brand-divider">
                <div className="space-y-4"><p className="text-4xl font-display font-light">{content.trust.stats.cities}</p><p className="text-[10px] uppercase tracking-widest font-bold text-brand-secondary">Cities</p></div>
                <div className="space-y-4"><p className="text-4xl font-display font-light">{content.trust.stats.states}</p><p className="text-[10px] uppercase tracking-widest font-bold text-brand-secondary">States</p></div>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="aspect-[4/5] bg-brand-bg relative overflow-hidden">
              <img src={content.trust.imageUrl} alt="LUXARDO FASHION Scale" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </motion.div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-16 gap-y-20 max-w-7xl mx-auto">
            {content.standards.items.map((item: any, i: number) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="space-y-6">
                ...
                <p className="font-sans text-brand-secondary leading-relaxed font-light">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
              ...
              <div className="space-y-6">
                ...
                <h2 className="text-4xl md:text-5xl font-display tracking-tight leading-tight">
                  {content.collaboration.heading.split(' ').map((word: string, i: number) => (
                    <React.Fragment key={i}>{i === content.collaboration.heading.split(' ').length - 1 ? <span className="italic text-brand-white/70">{word}</span> : word + ' '}</React.Fragment>
                  ))}
                </h2>
              </div>
              <p className="text-xl font-sans text-brand-white/80 leading-relaxed font-light">{content.collaboration.text}</p>
              <div className="pt-8 space-y-6">
                ...
                <ul className="space-y-4 font-sans font-light text-brand-white/90">
                  {content.collaboration.partners.map((partner: string, i: number) => (
                    <li key={i} className="flex items-center gap-4"><div className="w-1.5 h-1.5 bg-brand-white/50 rounded-full" />{partner}</li>
                  ))}
                </ul>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="aspect-[3/4] bg-brand-white/5 relative overflow-hidden">
              <img src={content.collaboration.imageUrl} alt="Premium Retail Space" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 bg-brand-white">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            {error && <div className="p-4 bg-red-50 border border-red-100 text-red-600 text-sm font-sans">{error}</div>}
            ...
            <div className="pt-8">
              <button type="submit" disabled={loading} className="btn-primary w-full py-6 flex items-center justify-center gap-4 text-[11px] uppercase tracking-[0.3em] font-bold disabled:opacity-50">
