...
export default function AdminPoliciesPage() {
  ...
  const handleReset = () => {
    if (window.confirm('Are you sure you want to reset all policies to defaults? This cannot be undone.')) {
      ...
      <div className="sticky top-0 z-30 bg-brand-bg/90 backdrop-blur-xl border-b border-brand-divider -mx-4 px-6 py-8 mb-16 transition-all shadow-sm">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <button 
              onClick={handleReset}
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-8 py-4 border border-brand-divider hover:border-brand-black hover:bg-brand-bg transition-all text-[10px] font-bold uppercase tracking-[0.2em] bg-white"
            >
            ...
            <button 
              onClick={handleSave}
              disabled={isSaving}
              className="flex-1 md:flex-none flex items-center justify-center gap-3 px-10 py-4 bg-brand-black text-brand-white hover:bg-brand-secondary transition-all text-[10px] font-bold uppercase tracking-[0.2em] disabled:opacity-50 min-w-[200px] shadow-xl shadow-brand-black/10"
            >
