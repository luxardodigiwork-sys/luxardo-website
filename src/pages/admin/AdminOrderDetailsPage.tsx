  ...
  const handleSaveTracking = () => {
    ...
    try {
      ...
      const updatedOrders = orders.map(o => 
        o.id === order.id ? { ...o, trackingId: trackingInput } : o
      );
      storage.saveOrders(updatedOrders);
      setOrder({ ...order, trackingId: trackingInput });
    } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      <div className="flex items-center gap-4">
        <Link to="/admin/orders" className="p-2 hover:bg-brand-bg rounded-full transition-colors">
          <ArrowLeft size={24} />
        </Link>
        <div>
          ...
          <div className="bg-white border border-brand-divider p-6 shadow-sm space-y-6">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-brand-secondary mt-2">
                        Qty: <span className="text-brand-black font-medium">{item.quantity}</span> × {formatINR(item.price)}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-sans font-medium text-brand-black">{formatINR(item.price * item.quantity)}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="pt-4 border-t border-brand-divider space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-brand-black">{formatINR(order.totalAmount)}</span>
              </div>
              <div className="flex justify-between text-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                {(order.status === 'shipped' || order.status === 'delivered' || order.trackingId) && (
                  <div className="space-y-2 pt-4 border-t border-brand-divider mt-4">
                    <p className="text-brand-secondary text-xs mb-1">Tracking ID</p>
                    <div className="flex gap-2">
                      ...
                      <button 
                        ...
                        className="btn-primary px-4 py-2 text-xs whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Save
                      </button>
                    </div>
